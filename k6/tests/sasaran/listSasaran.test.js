...
const sasaranTrend = new Trend('sasaran_duration');
...
export default function (data) {
    ...
    group('GET Sasaran', function () {
        ...
        for (let page = 1; page <= totalPage; page++) {
            const res = http.get(
                `${BASE_URL}/sasarans?page=${page}&limit=100`,
                authParams
            ...
            sasaranTrend.add(res.timings.duration);
...
export function handleSummary(data) {
    return {
        [`Report_Performance/Sasaran/Sasaran_limit100(${getTimestamp()}).html`]:
            htmlReport(data),
    };
}